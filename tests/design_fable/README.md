...
- viewBox: `0 0 1200 560`（横長 約2.1:1）
- チーム帯: x=162〜970、チーム略称は全角6文字（例: ソフトバンク）まで
- チームロゴ: 半径64（直径128。帯の高さ158の約8割）。丸い枠線・背景の円は付けず、透過ロゴをそのまま描画する想定。モックアップの野球ボールはプレースホルダー
- 得点スロット: x=780〜970（帯の右端いっぱい）、数字はスロット内で左右中央揃え
- スロット左端 x=780 に薄い仕切り線
- ステータス欄（ベース・カウント・アウト）: x=980〜1187、中心 x=1084
