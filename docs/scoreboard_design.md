...
共通ナビゲーションは上部ボタンを並べず、左側のサイドメニューに `HOME`、`見る`、`動かす`、`設定` を格納する。サイドメニューは、各ページ上部左に置く横3本線のメニューアイコンボタンで開く。アイコンは追加ライブラリ（@mui/icons-material）を導入せず、インラインSVGで描く。
...
- 拡大率は数値入力でも変更でき、ドラッグ操作や端リサイズで値が変わった場合は入力欄にも反映する。
- Viewer Pageで位置、拡大率、表示/非表示を最後に調整したスコアボードが、重なったとき一番前に出る。
- 選択中スコアボードを端末ごとに非表示にできる。非表示中も位置対象メニューには残し、選択して再表示できる。
...
- 操作ボタンは種類ごとに色を使い分ける。ボールと四球は緑、ストライクとファウルは黄、ヒットは青、HRは赤を基本にする。
...
### 2.6 UI Design（全ページ共通のダークテーマ）

アプリの全ページのUI（Home / Viewer / Control List / Score Input / Settings / Preset Reorder と各メニュー・ダイアログ）は、フットボール版（`C:\Users\riyok\code\OoAB_Football` の `docs/scoreboard_design.md` 2.6）で採用したダークテーマと同系のデザイントークンで統一する。
標準ボタンと入力欄はMUI v9の標準ダークテーマの外観を採用し、特定の色へ強制的に合わせない。ページ背景・カード・文字と、用途固有の強調色だけを共通トークンとして扱う。
各画面のレイアウト（配置・ボタン構成）は野球版の現行構成を維持し、配色とカード体裁だけを差し替える。
スコアボード本体（3.2 Broadcast LED スタイル）はこのテーマの対象外とし、見た目を変更しない。

基本トークン（フットボール版 `src/client/tokens.js` から移植する）:

| 項目 | 値 |
| --- | --- |
| ページ背景 | `#0c0f14` |
| カード/パネル背景 | `#10151f`、枠線 1px `#232c3d`、角丸 14px |
| カード内の区切り線 | 1px `#232c3d` |
| 基本文字色 | `#e8eef8` |
| 控えめ文字色（補助テキスト・-1ボタン等） | `#93a3bb` |
| グループ見出しラベル | 太字11px、色 `#7e8ba3`、字間 0.08em |
| 危険系ボタン（スコアリセット、削除等） | 標準ボタン形状で文字色 `#f87171` |
| リンク色 | `#38bdf8`（ホバー `#7dd3fc`） |
| フォント | 現行のまま（Noto Sans JP 系） |

用途固有の強調色（野球版固有の配色。ダーク背景でも読みやすい明度へ調整したうえで、個別指定を続ける）:

- 操作ボタンの種類別の色分け（ボール・四球=緑、ストライク・ファウル=黄、ヒット=青、HR=赤を基本とする。2.4参照）
- チームカラー・文字色の設定値表示（カラーピッカー、プレビュー）

オーバーレイメニュー（Drawer。編集メニュー・選手名メニュー等）:

- 背景 `#10151f`、枠線 `#232c3d` のカード体裁とし、背面に黒45%のスクリム（クリックで閉じる）を敷く。閉じるボタンは維持する。
- タブ・入力欄の選択状態はMUI標準の状態表現を使う。

各ページへの適用方針:

- Home Page: ページ背景・カードを上記トークンで構成する。
- Viewer Page: 表示プロパティパネルをカード＋標準ボタン・入力欄のトークンで作る。配信で映る領域（背景色部分）には余計な装飾を置かない。
- Control List Page: スコアボード一覧をカードで並べ、削除ボタンは危険系の配色にする。
- Settings Page / Preset Reorder Page: プリセットカード・設定フォームをカード＋入力欄トークンで構成する。
- 確認ダイアログ: カード背景・枠線を流用し、削除などの実行ボタンは危険系の配色にする。
