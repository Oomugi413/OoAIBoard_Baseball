...
- 「先攻勝利で試合終了」「後攻勝利で試合終了」ボタンを置き、確認ダイアログを経て試合終了状態を確定する。演出の詳細は3.4を参照。
...
- 三振演出は3秒固定で表示し、フォントサイズはスコアボード縦幅の約8割にする。ホームラン演出のフォントサイズは従来どおりにする。
...
- ホームラン・三振の演出は、チームカラーで塗った不透明な板が上からフェードインする全面オーバーレイとして表示する。板の色は、ホームランは攻撃側、三振は守備側のチームカラーを使う。
- ホームラン演出は、現在の得点表示と同じ大きさの `HOME RUN` を中央よりやや上、バッター名をその下にひとまわり小さく配置する。
- 三振演出は、`K`（見逃し三振は左右反転）を現在のフォントサイズの50%にし、空いたスペースにバッター名と、その打席の投手の空振り三振・見逃し三振の合計を表示する。
- バッター・ピッチャー交代時は、対戦選手表示欄の該当行（選手名、打順/守備位置の四角、成績）をフェードアウト→フェードインで切り替える。
- 得点・カウントの数字は、スロットや方向幕のように1段ずつ回転して切り替える。ただし打席結果ボタンによってカウントが0-0に戻る場合だけは、1段ずつではなく瞬時に切り替える。ホームラン時の桁送りはオーバーレイの下に隠れてよい。
- 攻守交代時は、10秒間ランナー、アウト、カウントを隠し、その領域に "Mid 1st"/"End 2nd" のようなラベルを表示する。表示中は上部の対戦選手表示欄と左のイニング表示欄をスコアボード本体へ折りたたむアニメーションを再生する。
- 試合終了時は、敗者チームの帯を設定に関わらず灰色・白文字にし、攻守交代時と同じ領域に "Final" を永続的に表示する。対戦選手表示欄も折りたたんだままにする。
