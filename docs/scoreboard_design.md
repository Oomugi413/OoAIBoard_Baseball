...
採用するビジュアルデザインは `design_claude_opus.jpg`（詳細は 4.2）。
...
参考画像: `design_claude_opus.jpg`

![新スコアボードデザイン案 (Broadcast LED)](design_claude_opus.jpg)
...
- カウントは `B - S` の微小ラベル付きで `3-2` のように大きく表示する。
- アウトはLED風のドット（点灯=赤グロー、消灯=グレー枠）で表す。
- ABSの残チャレンジ数は、各チーム帯の左端に小さな縦ピップで示す。
