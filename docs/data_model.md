...
- default board scale
- board scales
- per-device board size derived from board scale
- viewer client id
- board layout mode
- board positions
...
- Viewer Pageで最後に位置、拡大率、サイズを調整した順番を端末ごとに保存する。
...
閲覧画面の表示プロパティ（背景色・スコアボードごとの拡大率・サイズ・表示位置・最後に調整した順番）は、サーバーではなく各ブラウザ内にだけ保存する。
