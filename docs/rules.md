...
- 現時点で12.までが実装済みで、`2.3.2 React + MUI 移行`に完全実装が済んだ要素を移動する。
...
13. 2.2.2の実装希望リストを完了する。（打者選手名変更時の当該打順成績リセット、スマホ幅に合わせたスコア入力画面縮小、ファウル、カウントRS、四球、死球、投球/打席結果ボタンの画面幅に応じた自然折り返し、ボタン色変更を実装。単体テスト、Viteビルド確認済み）
...
### 3.1 Coding Rules for Fable

Claude Code によってコーディングする際専用のルール。執筆時点では、このルールをすべて無視する。
...
- サブエージェントは、新たなサブエージェントを作ってはいけない。
...
- 閲覧画面の完全非表示UIモードを将来追加できる余地を残す。（第1期17の引き継ぎ、優先度最下位）
