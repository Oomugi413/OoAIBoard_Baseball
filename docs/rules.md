...
- [未] GSAPと@gsap/reactを導入し、演出アニメーションのタイムライン基盤を作る。
...
- [未] 攻守交代の中間表示: チェンジ操作から10秒間、イニングの▲▼と数字以外（ランナー、アウト、カウント）を隠し、カウントが表示されていた領域に"Mid 1st"/"End 2nd"のようなラベルを表示する。表示中は上部の対戦選手表示欄をアニメーションでスコアボード本体へ折りたたむ。
- [未] 試合終了演出: 「試合終了」操作（先攻/後攻それぞれの勝利ボタン、要確認）で、敗者チームの帯を設定に関わらず灰色・白文字にし、攻守交代の中間表示と同じ領域に"Final"を永続的に表示する。上部の対戦選手表示欄も折りたたんだままにする。スコアリセットで解除する。
- [一部] 投手ごとの三振の集計を成績データに追加し、三振演出の表示数値に使う。スコアリセットで0に戻す対象に含める。（投手ごとの三振の集計を成績データに追加のみ実装）
- [未] Viewer Pageのスコアボード表示位置の基準点を、ボードの右下に変更する（攻守交代/試合終了時の折りたたみでボード高さが縮んでも、右下の位置がずれないようにするため）。
