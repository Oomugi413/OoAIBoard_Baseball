...
- [済] GSAPと@gsap/reactを導入し、演出アニメーションのタイムライン基盤を作る。
- [済] Viewer Pageのスコアボード表示位置の基準点を、ボードの右下に変更する（攻守交代/試合終了時の折りたたみでボード高さが縮んでも、右下の位置がずれないようにするため）。
- [一部] 投手ごとの三振の集計を成績データに追加し、三振演出の表示数値に使う。スコアリセットで0に戻す対象に含める。（投手ごとの三振の集計を成績データに追加のみ実装）
- [未] ホームラン演出: チーム背景色で塗った不透明な板が上からフェードインし、現在の得点文字と同じフォントサイズの"HOME RUN"を中央よりやや上、バッター名をその下にひとまわり小さく配置する。
- [未] 三振演出: ホームランと同じ板・フェード機構を使い、"K"のフォントサイズを現在の50%にして、空いた領域にバッター名と、その打席の投手の三振集計「(5)」のような数字を表示する。
- [未] バッター交代のフェード: 打者が交代した瞬間に、選手名・打順の四角・成績のテキストをフェードアウト→フェードインで切り替える。
- [未] 得点・カウントの桁送り演出: 得点とボール-ストライクカウントを、スロット/方向幕のように1段ずつ回転させて更新する。打席結果ボタンの結果としてカウントが0-0に戻る場合だけは、1段ずつではなく瞬時に切り替える（カウントRS、スコアリセットによるリセットは対象外で、通常どおり1段ずつ回す）。ホームラン時の桁送りは、演出オーバーレイの下に隠れてよい。
