...
12. [済] 複数端末操作の競合処理を確認する。後勝ちの動作と、SSE更新が編集中フォームの入力を消さないことを確認する。（設定画面、編集メニュー、選手名メニューで、別経路のスコア操作によるSSE更新後も入力中の下書きが保持されることを実ブラウザで確認。スコアボード名更新は後から届いた保存内容が最終状態になることをAPIで確認。単体テスト、Viteビルド確認済み）
