...
11. [済] 戻る、進むのキーボードショートカット（Ctrl+Z / Cmd+Z、Ctrl+Shift+Z / Cmd+Shift+Z）を追加する。（Score Input Pageでのみ有効化し、入力欄・選択欄にフォーカスがある間は反応しない。単体テスト、Viteビルド確認済み）
12. [済] 複数端末操作の競合処理を確認する。後勝ちの動作と、SSE更新が編集中フォームの入力を消さないことを確認する。（設定画面、編集メニュー、選手名メニューで、別経路のスコア操作によるSSE更新後も入力中の下書きが保持されることを実ブラウザで確認。選手名メニューは実際に編集した項目だけを更新し、未編集の球数・成績を古いスナップショットで巻き戻さない方式へ修正。スコアボード名更新は後から届いた保存内容が最終状態になることをAPIで確認。単体テスト、Viteビルド確認済み）
